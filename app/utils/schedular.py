...
def daily_scheduler(db, schedule):
    ...
    if schedule.action == 'SCAN':
        # run scan
        response = requests.post(EXPERIENCE_SERVICE_URL+'/experience-service/v1/bulk-scan',
                                 data=json.dumps({'reference': schedule.reference}), headers={"Content-Type": "application/json"})
        print(str(response), file=sys.stdout)
        db.add(schedule)
        db.commit()

    if schedule.action == 'URL':
        requests.post(schedule.action_params, data={})
